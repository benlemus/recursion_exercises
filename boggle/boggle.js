...
function findFromChar(board, word, y, x, seen) {
  ...
  if (y > 0 && findFromChar(board, word.slice(1), y - 1, x, seen)) return true;
  if (y < 4 && findFromChar(board, word.slice(1), y + 1, x, seen)) return true;
  if (x > 0 && findFromChar(board, word.slice(1), y, x - 1, seen)) return true;
  if (x < 4 && findFromChar(board, word.slice(1), y, x + 1, seen)) return true;
...
function find(board, word) {
  ...
  for (let y = 0; y < 5; y++)
    for (let x = 0; x < 5; x++)
      if (findFromChar(board, word, y, x, new Set())) return true;
